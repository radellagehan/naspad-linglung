...
public class MainFrame extends JFrame {
    ...
    public MainFrame() {
        ...
        // Layout: Seller and Buyer in separate tabs
        BuyerPanel buyerPanel        = new BuyerPanel(0); // legacy frame — userId not needed here
        RecommendationPanel recPanel = new RecommendationPanel(buyerPanel);
        SellerPanel sellerPanel      = new SellerPanel();

        JPanel sellerCol = wrapColumn("🏪 Seller Dashboard",      sellerPanel, new Color(100, 180, 255));
        JPanel buyerCol  = wrapColumn("👑 Buyer Dashboard",     buyerPanel,  new Color(255, 105, 180));
        JPanel recCol    = wrapColumn("✦ Top Recommendations",  recPanel,    new Color(200, 140, 255));

        JSplitPane buyerSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, buyerCol, recCol);
        buyerSplit.setResizeWeight(0.67);
        buyerSplit.setDividerSize(4);
        buyerSplit.setBackground(StyleUtil.BG_DARK);

        tabs.addTab("👑 Buyer Dashboard", buyerSplit);
        tabs.addTab("🏪 Seller Dashboard", sellerCol);
