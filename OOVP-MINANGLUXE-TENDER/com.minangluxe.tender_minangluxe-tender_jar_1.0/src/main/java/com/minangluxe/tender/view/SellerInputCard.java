...
public class SellerInputCard extends JPanel {
    ...
    private int dbRequestId = -1; // DB primary key of the selected tender_request row
    ...
    public SellerInputCard(Seller seller) {
        ...
        JButton infoBtn = new JButton("Edit Profile & Map ⚙️");
        StyleUtil.styleLinkButton(infoBtn);
        infoBtn.addActionListener(e -> SellerContactDialog.showFor(this, seller, this::refreshProfile));
        
        contactLbl = new JLabel("📞 " + (seller.getContactId().isEmpty() ? "—" : seller.getContactId()));
    ...
    private void refreshProfile() {
        contactLbl.setText("📞 " + (seller.getContactId().isEmpty() ? "—" : seller.getContactId()));
        revalidate();
        repaint();
    }
    ...
    /** Called by SellerPanel so the card knows which DB row to link the offer to. */
    public void setDbRequestId(int id) {
        this.dbRequestId = id;
    }
    ...
    private void submitOffer() {
        ...
        try {
            ...
            // Persist to DB so Buyer (other process) can see the offer
            if (dbRequestId > 0) {
                com.minangluxe.tender.util.DatabaseConnection.executeUpdate(
                    "INSERT INTO tender_offers (request_id, seller_name, product_name, price, quantity, rating) VALUES (?, ?, ?, ?, ?, ?)",
                    dbRequestId, seller.getName(), name, price, qty, rating);
            }

            // Also notify in-process listeners (same JVM)
            Offer offer = new Offer(seller, new Product(name, name, qty, qty), price, qty, rating, "", currentRequest);
            ...
            javax.swing.Timer t = new javax.swing.Timer(2000, e -> { submitBtn.setText(StyleUtil.ICON_SEND + " Send Offer"); StyleUtil.styleActionButton(submitBtn, StyleUtil.GOLD_PRIMARY); });
